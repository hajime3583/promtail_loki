...
echo "http_listen_port=$3" >> /root/test.txt
echo "grpc_listen_port=$4" >> /root/test.txt
echo "positions_file=$5" >> /root/test.txt
echo "client1_url=$6" >> /root/test.txt
echo "client2_url=$7" >> /root/test.txt
echo "job_name=$8" >> /root/test.txt
echo "job=$9" >> /root/test.txt
echo "log_path=${10}" >> /root/test.txt
conf_dir="/etc/promtail/conf"
...
for dir in "${dirs[@]}"; do
    if [ ! -d "$dir" ]; then
        ...
        #chown root:root "$dir"  # 所有者をroot:rootに設定
